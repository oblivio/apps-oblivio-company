...
# Import database components from mdb_runtime
...
# Re-export JWT utilities from mdb_runtime for backward compatibility
from mdb_runtime.auth.jwt import decode_jwt_token

# decode_jwt_token is now imported from mdb_runtime.auth.jwt above
# Keeping this function for backward compatibility in this file only
# Import JWT utilities from mdb_runtime
from mdb_runtime.auth.jwt import decode_jwt_token
...
# Import auth dependencies from mdb_runtime
from mdb_runtime.auth.dependencies import (
    get_authz_provider,
    get_current_user,
...
# Re-export auth dependencies from mdb_runtime (these are already imported above)
...
# require_permission is imported from mdb_runtime.auth.dependencies above
# This function is kept for backward compatibility but delegates to mdb_runtime
...
# Wrapper for get_experiment_db that provides get_scoped_db
async def get_experiment_db(request: Request) -> ExperimentDB:
    """
    FastAPI Dependency: Provides an ExperimentDB instance.
    
    This wraps mdb_runtime.database.get_experiment_db and provides
    the get_scoped_db function from the application layer.
    """
    from mdb_runtime.database.abstraction import get_experiment_db as _get_experiment_db
    return await _get_experiment_db(request, get_scoped_db_func=get_scoped_db)